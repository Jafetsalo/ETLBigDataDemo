...
try:
    ...
    reporte = f"Tiempo de ejecución: {tiempo_fin - tiempo_inicio} segundos para la ejecución de {registros} registros"
    ...
    # -------------------------------------------------------------------------
    command   = '''SELECT pg_size_pretty(pg_database_size(%s));''' #REPARACION TRABAJO DE ESTUDIANTE INICIO 
    resultado = cursor.execute(command,(v_database,)) #REPARACION TRABAJO DE ESTUDIANTE FIN
    record    = cursor.fetchall()
