...
def cargarMunicipio(conn, cur, contador, id_pais, codigo_dep, codigo_mun, municipio):
    ...
    #
# INICIO TRABAJO ESTUDIANTE   
#-----------------------------------------------------------------------------
# Función:  Cargar id_región, vamos a hacer update a la tabla operaciones para agregar el id_region respectivo a partir del departamento
#-----------------------------------------------------------------------------
def cargar_id_region(conn, cur, numero_region, codigo_dep, id_pais):
    try:
        ...
        print(codigo_region, codigo_departamento, end="\n") 
    
        command = '''UPDATE public.operaciones SET id_region = %s 
        WHERE id_departamento = %s;'''

        cur.execute(command, (codigo_region, codigo_departamento))
        conn.commit()
    except (Exception, Error) as error:
        print("Error en actualización de operations: ", error)
        sys.exit("Error: Update Operations table!")
    finally:
        return    
    # Fin función cargar_id_region 
...
try:
    ...
    for row in registros:
        ...
        #INICIO TRABAJO ESTUDIANTE
        cargar_id_region(connection,cursor,codigo_region,codigo_dep,id_pais )
        #FIN TRABAJO ESTUDIANTE
